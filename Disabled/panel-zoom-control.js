  ...
  */
  ...
  const ACTIONS_SEARCHTEXT = {
    ...
    cancel_search: {
      title: "Cancel search",
      script: function (target, webview) {
        webview.parentElement.parentElement
          .querySelector("#pamodsearchTextBar")
          .remove();
  ...
  const ACTIONS = {
    ...
    terminate: {
      title:
        "Double-click kills the panel to free memory. WARNING! This will also kill any tabs using the same process.",
  ...
  function updateZoomLabel(webview) {
    const panelZoom = webview.parentElement.parentElement.querySelector(
      ".zoom-reset"
    );
    if (!panelZoom) {
  ...
  function createActionButton(action, webview) {
    ...
    newBtnDiv.className =
      action.display_class + " button-toolbar mod-panel-action";
