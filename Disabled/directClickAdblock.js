...
window.gnoh = Object.assign(window.gnoh || {}, {
  ...
  decode: {
    url: function (str) {
      return !str
        ? str
        : decodeURIComponent(str.replace(/%(?![0-9][0-9a-fA-F]+)/g, "%25"));
    },
  },
  ...
  getFormData: function (formElement) {
    ...
    const inputElements = Array.from(formElement.elements).filter(function (
      field
    ) {
      if (field.name) {
  ...
  alert: function (message, okEvent) {
    ...
    this.dialog("Gnoh", message, [buttonOkElement], {
      width: 400,
...
(function () {
  ...
  function showDialogImportNew(blockList) {
    if (blockList.last_fetch_result === "success") {
      const importNewBlockerListHTML = [
        "<h3>" + blockList.unsafe_adblock_metadata.title + "</h3>",
        '<p class="info" style="word-break: break-all">' +
          blockList.source_url +
          "</p>",
        "<p>Valid rules: " +
          blockList.rules_info.valid_rules +
          "<br>Invalid rules: " +
          blockList.rules_info.invalid_rules +
          "<br>Unsupported rules: " +
          blockList.rules_info.unsupported_rules +
          "</p>",
      ].join("");
      ...
      gnoh.dialog(
        "Import New Blocker List",
        importNewBlockerListHTML,
        [buttonOkElement, buttonCancelElement],
        {
          width: 500,
    ...
    vivaldi.contentBlocking.onRulesFetchComplete.removeListener(
      showDialogImportNew
    );
  }
  ...
  chrome.tabs.onUpdated.addListener(function (tabId, changeInfo, tab) {
    ...
    if (
      ...
      if (tab.windowId === vivaldiWindowId) {
        vivaldi.contentBlocking.addKnownSourceFromURL(
          vivaldi.contentBlocking.RuleGroup.AD_BLOCKING,
          blockListURL,
          function (blockListId) {
            if (!chrome.runtime.lastError) {
              vivaldi.contentBlocking.onRulesFetchComplete.addListener(
                showDialogImportNew
              );
            } else {
